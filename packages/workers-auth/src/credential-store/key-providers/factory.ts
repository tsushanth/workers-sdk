 ...
 */
let testProviderFactory: ((serviceName: string) => KeyProvider) | undefined;

/**
 * Inject a `KeyProvider` factory for testing. Pass `undefined` to restore
 * the real per-platform resolution.
 */
export function setKeyProviderFactoryForTesting(
	factory: ((serviceName: string) => KeyProvider) | undefined
): void {
 ...
 */
export function resolveKeyProvider(serviceName: string): KeyProviderResolution {
	if (testProviderFactory !== undefined) {
		return { kind: "available", provider: testProviderFactory(serviceName) };
	}
	...
	switch (process.platform) {
		case "darwin":
			...
			return {
				kind: "available",
				provider: new MacSecurityKeyProvider(serviceName),
			};
		...
		case "win32":
			if (findKeyringBinding() !== null) {
				return {
					kind: "available",
					provider: new NapiKeyringKeyProvider(serviceName),
				};
			}
			return {
				kind: "needs-install",
				install: installKeyringBindingSync,
				afterInstall: () => new NapiKeyringKeyProvider(serviceName),
			};
