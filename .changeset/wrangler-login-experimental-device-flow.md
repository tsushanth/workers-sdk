...
`--callback-host` and `--callback-port` are ignored when `--experimental-device` is set, since this flow does not start a local callback server. The endpoint used by the device flow can be overridden with the new `WRANGLER_DEVICE_AUTH_URL` environment variable.
